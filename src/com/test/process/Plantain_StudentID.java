...
public class Plantain_StudentID {
	...
	public Mat imgPlan_StuID(Mat boxImgPlan_stuID) {
		...
		Imgcodecs.imwrite("./src/img/id/before"+rd.nextInt(10)+".jpg", boxImgPlan_stuID);
...
//			System.err.println("TwoBlack: "+rect);
			if (rect.area() > 200 && rect.area() < 600) {
				if (((rect.y > 0 && rect.y < 60) || (rect.y > 280 && rect.y < hBoxP_StuID)) && (rect.x < 38)
						&& (rect.width > 13 && rect.width < 25) && (rect.height > 13 && rect.height < 25)) {
					rectTree.add(rect);
				}
			}
		}
	...
	public String getCodeID() {
		List<MatOfPoint> contours = MatProcess.getContour(imgPlan_StuID(MatProcess.threshold(matPathImg,wBoxPlan_StuID, hBoxP_StuID)));
		Set<Rect> rectTree = new TreeSet<Rect>(SortPlantainY.SORT_PLANTAIN);
		for (int i = 0; i < contours.size(); i++) {
			Rect rect = Imgproc.boundingRect(contours.get(i));
//			System.out.println(rect);
			if (rect.area() > 130 && rect.area() < 750) {
				if ((rect.y > 0 && rect.y < hBoxP_StuID) && (rect.x > 0 && rect.x < wPlan_StuID)
						&& (rect.width > 9&& rect.width < 30) && (rect.height > 9 && rect.height < 30)) {
					rectTree.add(rect);
				}
			}
		}
	...
	public String getnumberRow(Rect rect, int size) {
		...
		while (count != -1) {
			...
			if (rect.y <= coordinates && rect.y >= coordinates - 30) {
	...
	public Mat cropImgResult(int xP_S, int yP_S, int wP_S, int hP_S) {
		Mat mat = new Mat(MatProcess.threshold(matPathImg,wBoxPlan_StuID, hBoxP_StuID), new Rect(xP_S, yP_S, wP_S, hP_S));

...
public class MatProcess {
	...
	public static Mat toThreshBinary(Mat src) {
		Mat roi_thresh = new Mat();
//		Imgproc.threshold(src, roi_thresh, thresh, 250, Imgproc.THRESH_BINARY_INV);
		Imgproc.adaptiveThreshold(src, roi_thresh, 255, Imgproc.ADAPTIVE_THRESH_GAUSSIAN_C,
...
//	---
	...
	public static Point pointAfterRotate(Point oldPoint, double angle, Point central) {
		double x = (oldPoint.x - central.x)*Math.cos(Math.toRadians(angle)) - (oldPoint.y - central.y)*Math.sin(Math.toRadians(angle));
		double y = (oldPoint.x - central.x)*Math.sin(Math.toRadians(angle)) + (oldPoint.y - central.y)*Math.cos(Math.toRadians(angle));
